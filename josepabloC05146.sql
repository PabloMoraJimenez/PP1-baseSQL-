...
--Como el proceso de muchos a muchos no tiene una función programada, entonces se debera crear una tabla de union con la tabla que contendra la foreign key

--Creación de la tabla de unión Producto
CREATE TABLE compra (
producto_id INT PRIMARY KEY, --establece producto_id como primary key
factura_numero INT --y agrega una columna mas llamada factura_numero
);

--Establecer la clave foránea entre compra y producto
ALTER TABLE compra --permite modificar tabla compra
ADD CONSTRAINT FK_compra_producto --permite crear una foreign key
FOREIGN KEY (producto_id) --le indica cual columna es la foreign key de la tabla compra
REFERENCES producto(ID) --le indica cual columna primary key de otra tabla debe usar como foreign key

--Establecer la clave foránea entre compra y factura
AlTER TABLE compra --permite modificar tabla compra
ADD CONSTRAINT FK_compra_factura --permite crear una foreign key
FOREIGN KEY (factura_numero) --le indica cual columna es la otra foreign key de la tabla compra
REFERENCES factura(numero_factura) --le indica cual columna primary key de otra tabla debe usar como foreign key
--------------------------------------------------------------------------------------------------------------------------------
